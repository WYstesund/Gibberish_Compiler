...
class ProgramNode extends ASTnode {
    ...
    public void codeGen() {
        generateWithComment(".data", "Global Variable Declarations");
        generate(".align 4");
        globalScope = true;
        target = null;
        myDeclList.codeGen();
    ...
    public static boolean globalScope;
...
class DeclListNode extends ASTnode {
    ...
    public void codeGen() {
        if (ProgramNode.globalScope) {
            // In the global scope, we process all VarDecls first
            for (DeclNode node : myDecls) {
                if (node instanceof VarDeclNode) {
                    node.codeGen();
                }
            }
            ProgramNode.globalScope = false;

            // Then process all FnDecls
            generate(".text");
            for (DeclNode node : myDecls) {
                if (!(node instanceof VarDeclNode)) {
                    node.codeGen();
            ...
            // Make space for local vars, NameAnalysis will make offsets
            int localSpace = myDecls.size() * 4;
            generate("subu", SP, SP, localSpace);
        }
    }
...
class FnBodyNode extends ASTnode {
    ...
    public void codeGen() {
        ...
        // Position the FP
        int localSpace = myDeclList.getNumDecls() * 4;
        generate("addi", FP, SP, "" + (localSpace + 8));

        // Label the function body start
        generateWithComment("", "FCN BODY START");
...
class ExpListNode extends ASTnode {
    ...
    public void codeGen() {
        // push the args in opposite order
    ...
    public void tearDown() {
        // Brings the SP back below the locals space
        generate("addi", SP, SP, (4*myExps.size()));
    }
...
class VarDeclNode extends DeclNode {
    ...
    public void codeGen() {
        // VarDecls only produce code (other than making space)
...
class FnDeclNode extends DeclNode {
    ...
    public void codeGen() {
        // Preamble to function, including __start: if it is main()
        generate(".globl " + myId.name());
        generateLabeled(myId.name(), "", "METHOD ENTRY");
        if (myId.isMain())
            generateLabeled("__start", "", "");

        // Prologue, setting up function, and frame on stack
        genPush(RA);
        genPush(FP);
        ...
        // Epilogue: Restoring frame pointer/stack, then
        // returning to caller or exiting program
        String exitLabel = String.format("_%s_Exit", myId.name());
        generateWithComment("", "FUNCTION EXIT");
        generateLabeled(exitLabel, "", "");
        generate("lw", RA, String.format("0(%s)", FP));
        generateWithComment("move", "save control link", T0, FP);
        generateWithComment("lw", "restore FP", FP, String.format("-4(%s)", FP));
        generateWithComment("move", "restore SP", SP, T0);
        if (myId.isMain()) {
            generate("li", V0, "10");
            generateWithComment("syscall", "exit program");
        } else {
            generateWithComment("jr", "return to caller", RA);
        }
    }
...
class StructDeclNode extends DeclNode {
    ...
    public void codeGen() {
        // Do not need to implement for P6
    }
...
class PostIncStmtNode extends StmtNode {
    ...
    public void codeGen() {
        generateWithComment("", "INCREMENT");

        myExp.codeGen();
        genPop(T0);
        generate("addi", T0, T0, 1);
        genPush(T0);
        ((IdNode)myExp).getAddress();
        genPop(T1);
        genPop(T0);
        generate("sw", T0, "($t1)");
    }
...
class WriteStmtNode extends StmtNode {
    ...
    public void codeGen() {
        generateWithComment("", "WRITE");

        if (myExp instanceof StringLitNode) {
            generate(".data");
            String label = nextLabel();
            String strVal = ((StringLitNode) myExp).getStrVal();
            generateLabeled(label, ".asciiz", "", String.format("%s", strVal));
            generate(".text");
...
class IfStmtNode extends StmtNode {
    ...
    public void codeGen() {
        generateWithComment("", "IF");
        
        String endTarget = nextLabel();
        /**if (myExp instanceof LogicalExpNode) {
            // Each logical expression has its own behavior,
            // including short-circuiting.
            if (myExp instanceof AndNode) {
                ((AndNode) myExp).getExp1().codeGen();
                genPop(T0);
                generate("beq", T0, "$zero", endTarget);
                ((AndNode) myExp).getExp2().codeGen();
                genPop(T0);
                generate("beq", T0, "$zero", endTarget);
            } else if (myExp instanceof OrNode) {
                String condBodyTarget = nextLabel();
                ((OrNode) myExp).getExp1().codeGen();
                genPop(T0);
                generate("bne", T0, "$zero", condBodyTarget);
                ((OrNode) myExp).getExp2().codeGen();
                genPop(T0);
                generate("beq", T0, "$zero", endTarget);
                genLabel(condBodyTarget);
            }
        } else if (myExp instanceof BinaryExpNode) {
            // All other binary expressions have their own,
            // branch instruction, but have the same expression retrieval.
            ((BinaryExpNode) myExp).getExp1().codeGen();
            ((BinaryExpNode) myExp).getExp2().codeGen();
            genPop(T1);
            genPop(T0);

            if (myExp instanceof EqualsNode) {
                generate("bne", T0, T1, endTarget);
            } else if (myExp instanceof NotEqualsNode) {
                generate("beq", T0, T1, endTarget);
            } else if (myExp instanceof GreaterNode) {
                generate("ble", T0, T1, endTarget);
            } else if (myExp instanceof LessNode) {
                generate("bge", T0, T1, endTarget);
            } else if (myExp instanceof GreaterEqNode) {
                generate("blt", T0, T1, endTarget);
            } else if (myExp instanceof LessEqNode) {
                generate("bgt", T0, T1, endTarget);
            }
        } else {**/
            // unary expressions only, or literal values
            myExp.codeGen();
            genPop(T0);

            // checking to be sure it is equal to one
            generate("beq", T0, "$zero", endTarget);
        //}

        // Write out the body of the if statement
        myDeclList.codeGen();
        myStmtList.codeGen();
        // Write out the end label
        genLabel(endTarget);
    }
...
class IfElseStmtNode extends StmtNode {
    ...
    public void codeGen() {
        generateWithComment("", "IF/ELSE");
        
        String elseTarget = nextLabel();
        String endTarget = nextLabel();
        /**
        if (myExp instanceof LogicalExpNode) {
            // Each logical expression has its own behavior,
            ...
            if (myExp instanceof AndNode) {
                ((AndNode) myExp).getExp1().codeGen();
                genPop(T0);
                generate("beq", T0, "$zero", elseTarget);
                ((AndNode) myExp).getExp2().codeGen();
                genPop(T0);
                generate("beq", T0, "$zero", elseTarget);
            } else if (myExp instanceof OrNode) {
            ...
            if (myExp instanceof EqualsNode) {
                generate("bne", T0, T1, elseTarget);
            } else if (myExp instanceof NotEqualsNode) {
                generate("beq", T0, T1, elseTarget);
            } else if (myExp instanceof GreaterNode) {
                generate("ble", T0, T1, elseTarget);
            } else if (myExp instanceof LessNode) {
                generate("bge", T0, T1, elseTarget);
            } else if (myExp instanceof GreaterEqNode) {
                generate("blt", T0, T1, elseTarget);
            } else if (myExp instanceof LessEqNode) {
                generate("bgt", T0, T1, elseTarget);
            }
        } else {
**/            // unary expressions only, or literal values
            myExp.codeGen();
        ...
        // Write out the body of the if statement, then jump to end
        myThenDeclList.codeGen();
        myThenStmtList.codeGen();
        generate("j", endTarget);
        // Write out the else label and body
        genLabel(elseTarget);
...
class WhileStmtNode extends StmtNode {
    ...
    public void codeGen() {
        generateWithComment("", "WHILE");

        String entryTarget = nextLabel();
        String endTarget = nextLabel();

        // while condition check
        genLabel(entryTarget);
        myExp.codeGen();
        genPop(T0);
        generate("beq", T0, "$zero", endTarget);
        // loop body
...
class ReturnStmtNode extends StmtNode {
    ...
    public void codeGen() {
        ...
        if (FnDeclNode.isMain) {
            generate("li", V0, 10);
            generate("syscall");
        } else {
            generate("j", String.format("_%s_Exit", FnDeclNode.fcnName));
        }
    }
...
class IdNode extends ExpNode {
    ...
    public void codeGen() {
        // push global value, or local value
        if (mySym.isGlobal()) {
            generate("la", T0, "_" + myStrVal);
            generate("lw", T1, "($t0)");
            genPush(T1);
        } else {
            generate("addi", T0, FP, mySym.getOffset());
            generate("lw", T1, "($t0)"); 
            genPush(T1);
        }
    }
    ...
    public void getAddress() {
        // here we push the id's address for assignments
        if (mySym.isGlobal()) {
            generate("la", T0, "_" + myStrVal);
            genPush(T0);
        } else {
...
class DotAccessExpNode extends ExpNode {
    ...
    public void codeGen() {
        // We do not need to implement this in P6
    }
...
class CallExpNode extends ExpNode {
    ...
    public void codeGen() {
        // If there are arguments, generate them,
        // otherwise just jump to function
        if (myExpList != null) {
            myExpList.codeGen();
            generate("jal", myId.name());
            myExpList.tearDown();
        } else {
            generate("jal", myId.name());
        }
...
abstract class BinaryExpNode extends ExpNode {
    ...
    /**
     * codeGen generates the operators for the child nodes to operate on
     */
    public void codeGen() {
        myExp1.codeGen();
        myExp2.codeGen();
    ...
    public ExpNode getExp1() {
        return myExp1;
    }

    public ExpNode getExp2() {
        return myExp2;
    }
}
...
class NotNode extends UnaryExpNode {
    ...
    public void codeGen() {
        ...
        generate("bne", T0, "$zero", elseLabel);
        generate("li", T0, "1");
        generate("j", endLabel);
        genLabel(elseLabel);
        generate("li", T0, "0");
...
class AndNode extends LogicalExpNode {
    ...
    public void codeGen() {
        ...
        generate("beq", T0, "$zero", falseLabel);
        myExp2.codeGen();  
        String bothTrueLabel = nextLabel();
        genPop(T1);
        generate("beq", T0, "$zero", falseLabel);
        genPush(T1);
        generate("j", bothTrueLabel);
        genLabel(falseLabel);
        generate("li", T0, "0");
        genPush(T0);
        genLabel(bothTrueLabel);
    }
}
...
class OrNode extends LogicalExpNode {
    ...
    public void codeGen() {
        ...
        generate("bne", T0, "$zero", trueLabel);
        myExp2.codeGen();  
        String bothFalseLabel = nextLabel();
        genPop(T0);
        generate("bne", T0, "$zero", trueLabel);
        genPush(T1);
        generate("j", bothFalseLabel);
        genLabel(trueLabel);
        generate("li", T0, "1");
        genPush(T0);
        genLabel(bothFalseLabel);
    }
}
...
class EqualsNode extends EqualityExpNode {
    ...
    public void codeGen() {
        if (myExp1 instanceof StringLitNode) {
            String str1 = ((StringLitNode) myExp1).getStrVal();
            String str2 = ((StringLitNode) myExp2).getStrVal();
            if (str1.equals(str2)) {
                generate("li", T0, "1");
                genPush(T0);
            } else {
            ...
            generate("bne", T0, T1, label1); // if
            generate("li", T0, "1"); // then
            generate("b", label2);
            genLabel(label1); // else
            generate("li", T0, "0");
